...
class Buscador:
    ...
    def buscar_e_indexar(self, url, data_publicacao):
        conteudo_pagina = self.baixar_pagina(url)
        if conteudo_pagina:
            self.indexar_pagina(url, conteudo_pagina, data_publicacao)
    ...
    def buscar_e_classificar(self, termos_buscados):
        ...
        self.paginas.sort(key=lambda pagina: (pagina.calcular_pontuacao_termos(termos_buscados),
                                               ...
                                               pagina.links_recebidos),
                          reverse=True)

    def salvar_links(self, nome_arquivo):
        with open(nome_arquivo, 'a') as arquivo:  # Modificado para 'a' (append)
